...
class Control:
    ...
    def control_cnn(self):
        ...
        conv1 = tf.keras.layers.Conv1D(filters=16, kernel_size=3, padding='same',activation='relu')(input_layer)
        maxp1 = tf.keras.layers.MaxPool1D(2,2,padding='valid')(conv1)

        conv2 = tf.keras.layers.Conv1D(filters=32, kernel_size=3, padding='same',activation='relu')(maxp1)
        maxp2 = tf.keras.layers.MaxPool1D(2,2,padding='valid')(conv2)

        flat = tf.keras.layers.Flatten()(maxp2)
        
        d1 = tf.keras.layers.Dense(1024,kernel_initializer="normal",activation="relu")(flat)
        d2 = tf.keras.layers.Dense(512,kernel_initializer="normal",activation="relu")(d1)
