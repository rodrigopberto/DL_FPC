...
class ComplexModel:
    ...
    def complex_cnn(self):
        ...
        conv11 = CompConv1D(filters=16, kernel_size=5, strides=1, padding='same', activation='relu')(input_layer)
        conv12 = CompConv1D(filters=16, kernel_size=5, strides=1, padding='same', activation='relu')(conv11)
        mp_1 = tf.keras.layers.MaxPool1D(pool_size=2, padding='valid')(conv12)

        conv21 = CompConv1D(filters=32, kernel_size=5, strides=1, padding='same', activation='relu')(mp_1)
        conv22 = CompConv1D(filters=32, kernel_size=5, strides=1, padding='same', activation='relu')(conv21)
